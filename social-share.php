...
?>
    <div class="toggle-switch">
        <input type="checkbox" name="<?php echo esc_attr($args['name']); ?>" id="<?php echo esc_attr($args['name']); ?>" value="1" <?php checked(1, $option, true); ?> />
        <label for="<?php echo esc_attr($args['name']); ?>"></label>
    </div>
    <style>
        .toggle-switch {
            position: relative;
...
function add_social_share_icons($content)
{
    ...
    foreach ($social_networks as $network => $svg) {
        ...
        if ($option == 1) {
            if ($network === 'email') {
                // Create a mailto link with a default subject and body
                $subject = rawurlencode("I wanted to share this with you");
                $body = rawurlencode("Check out this page: " . $url);
                $href = "mailto:?subject={$subject}&body={$body}";
                $target = " target='_blank'";
                $onclick = "";
            } elseif ($network === 'print') {
                // For print, do not open a new tab; use an inline onclick event
                $href = "#";
                $target = ""; // omit target attribute
                ...
                switch ($network) {
                    case 'facebook':
                        ...
                        break;
                    case 'twitter':
                        $tweet = rawurlencode("Check out this page!");
                    ...
                    case 'reddit':
                        ...
                        break;
                    case 'pinterest':
                        $media = rawurlencode(get_the_post_thumbnail_url($post->ID, 'full'));
                        if (!$media) {
                            $media = rawurlencode("https://via.placeholder.com/150");
                        }
                        $description = rawurlencode(get_the_title($post->ID));
                        $href = "https://pinterest.com/pin/create/button/?url={$url}&media={$media}&description={$description}";
                        break;
                    default:
                        $href = "http://{$network}.com/share?url={$url}";
                        break;
                }
                $target = " target='_blank'";
                $onclick = "";
            }
            $html .= "<div class='{$network} social-icon'><a href='{$href}'{$target}{$onclick}>{$svg}</a></div>";
        }
    }
